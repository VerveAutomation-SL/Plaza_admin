...
const sortByCode = (arr: SubCategory[]) =>
  [...arr].sort((a, b) =>
    a.SCategory_code.localeCompare(b.SCategory_code)
  );
...
export default function BasicTableOneSubCategory() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setSubCategories(sortByCode(res.data));
      } catch (err) {
      ...
      <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/40 backdrop-blur-lg mt-80">
        <div className="bg-white px-8 py-6 rounded-xl shadow-xl border max-w-md w-full z-[99999]">
          <p className="mb-6 text-center text-lg font-semibold text-gray-800">
            Are you sure you want to delete <span className="font-bold">{sub.SCategory_name}</span>?
          </p>
          <div className="flex justify-center gap-4">
            <button
              onClick={async () => {
                try {
                  await deleteSubCategory({ SCategory_code: sub.SCategory_code });
                  setSubCategories(prev =>
                    sortByCode(prev.filter(item => item.SCategory_code !== sub.SCategory_code))
                  );
                  toast.dismiss(t.id);
                  toast.success("Subcategory deleted successfully", { position: "top-center" });
                } catch {
                  toast.dismiss(t.id);
                  toast.error("Failed to delete subcategory", { position: "top-center" });
                }
              }}
  ...
  const handleLocalUpdate = (updated: {
    ...
    setSubCategories(prev =>
      sortByCode(
        prev.map(item =>
          item.SCategory_code === updated.SCategory_code
            ? {
                ...item,
                SCategory_name: updated.SCategory_name,
                MainCategory: {
                  ...item.MainCategory,
                  mCategory_code: updated.MainCategory_code,
                },
              }
            : item
        )
