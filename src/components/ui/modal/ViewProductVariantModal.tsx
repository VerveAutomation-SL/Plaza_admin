...
interface ViewProductVariantModalProps {
  isOpen: boolean;
  onClose: () => void;
  barcode: string;
}

const ViewProductVariantModal: React.FC<ViewProductVariantModalProps> = ({ isOpen, onClose, barcode }) => {
  const [variant, setVariant] = useState<ProductVariant | null>(null);

  useEffect(() => {
    if (isOpen && barcode) {
      getProductByBarcode(barcode)
        .then((res) => setVariant(res?.product || null))
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/30 backdrop-blur-[32px]">
      <div
        onClick={(e) => e.stopPropagation()}
        className="relative w-full max-w-xl rounded-xl bg-white p-6 shadow-lg dark:bg-gray-900"
      >
        <button
          onClick={onClose}
...
export default ViewProductVariantModal;
